...
import type { CatalogTask, CompiledRoadmap, EventPack, LocalProgress, TaskDiff } from "@/domain-contracts";
...
export function ActionRoute({ roadmap, sourceCards, progress, taskDiff, rationaleByKey, readOnly = false, onCycleProgress }: Props) {
  ...
  return <section className="action-route" aria-labelledby="route-title">
    ...
    {selected && <TaskDrawer task={selected} sourceCards={sources} taskTitles={taskTitles} rationale={rationaleByKey[selected.rationale]} status={statusFor(selected, progress)} readOnly={readOnly} onClose={() => setSelected(undefined)} onCycle={() => onCycleProgress(selected.id)} />}
  </section>;
}

function TaskDrawer({ task, sourceCards, taskTitles, rationale, status, readOnly, onClose, onCycle }: { task: CatalogTask; sourceCards: Map<string, EventPack["sourceCards"][number]>; taskTitles: Map<string, string>; rationale?: string; status: string; readOnly: boolean; onClose(): void; onCycle(): void }) {
