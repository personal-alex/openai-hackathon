...
const introSeenKey = "life-navigator.intro-seen.v1";
...
export default function Home() {
  ...
  function understandEvent() { const id = inferScenarioId(statement); if (!id) { setError("This seeded demo currently supports “I’m expecting a child” and “I lost my job.” Choose one below to continue."); return; } setScenarioId(id); setContext({ facts: {} }); setProgress(emptyProgress); setLastDiff(undefined); setState("understood"); }
  function confirmEvent() { setState("planning"); }
  function answer(value: SeededValue | undefined) { if (!activeQuestion) return; const facts = { ...context.facts }; if (value === undefined) delete facts[activeQuestion.factId]; else facts[activeQuestion.factId] = value; const next = { facts }; const nextRoadmap = compileRoadmapPresentation(scenario.pack, next).roadmap; setLastDiff(diffRoadmaps(presentation.roadmap, nextRoadmap)); setContext(next); setProgress((current) => ({ progressStatusByTaskId: Object.fromEntries(Object.entries(current.progressStatusByTaskId).filter(([id]) => nextRoadmap.steps.some((task) => task.id === id))) })); }
  ...
  function reset() { localStorage.removeItem(storageKey); setState("entry"); setStatement(""); setScenarioId("expecting_child"); setContext({ facts: {} }); setProgress(emptyProgress); setLastDiff(undefined); setError(undefined); }
  ...
  return <main className={`app-shell action-shell${showIntro ? introExiting ? " intro-exiting" : " intro-pending" : ""}`}>
    ...
    <header className="app-header"><BrandMark /><div className="mode-badge" data-testid="seeded-ai-boundary"><span aria-hidden="true" />Seeded demo · deterministic</div></header>
    ...
    <section className="action-workspace" aria-label="Life Navigator Action Route workspace">
      <section className="conversation-pane" aria-labelledby="conversation-title"><p className="section-kicker">Planning companion · Israel · English</p><h1 id="conversation-title">Tell us what changed.</h1><a className="mobile-plan-link" href="#route-title">View your route</a><div className="conversation-thread">
        ...
        {state === "understood" && <article className="assistant-message"><p>{scenario.confirmationCopy}</p><p>Is that right?</p><div className="message-actions"><button className="primary-button" type="button" onClick={confirmEvent}>Yes, that’s right</button><button className="answer-button" type="button" onClick={() => setState("entry")}>Choose a different event</button></div></article>}
        {state === "planning" && <><article className="assistant-message">Your route is ready to refine. I’ll ask only what can change it.</article>{activeQuestion ? <article className="assistant-message active-question"><h2>{activeQuestion.presentation.prompt}</h2><QuestionInput question={activeQuestion} onAnswer={answer} /><details className="why-ask"><summary>Why are we asking this?</summary><p>{activeQuestion.presentation.rationale}</p></details></article> : <article className="assistant-message"><h2>{presentation.mode === "preview" ? "Your after-birth preview is ready." : "Your route is ready."}</h2>{presentation.mode === "preview" && <button className="primary-button" type="button" onClick={confirmBirth}>The child has now been born</button>}</article>}</>}
      </div>
