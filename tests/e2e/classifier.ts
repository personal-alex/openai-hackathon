...
export async function mockLiveClassifier(page: Page): Promise<{ calls: string[] }> {
  ...
  await page.route("**/api/ai/extract-event", async (route) => {
    ...
    const eventId = /expecting|pregnan|having.*baby|baby.*having/.test(normalized) ? "expecting_child" : /lost.*job|job.*lost|employment.*ended/.test(normalized) ? "job_loss" : null;
