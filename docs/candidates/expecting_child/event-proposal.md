...
| `ec_register_newborn_population_registry` | **Register the newborn — Population and Immigration Authority.** Immediately after birth: usually system-led after a hospital birth in Israel. The hospital submits the birth notice and the Population and Immigration Authority completes the Population Registry entry. | `{ "kind": "event_relative", "anchor": "event_date", "window": "immediate", "labelKey": "expecting_child.candidate.immediately_after_birth" }`; candidate applicability: `{ "kind": "confirmed_transition", "requiredFacts": [{ "factId": "event_stage", "equals": "birth_occurred" }] }` | `ec_piba_birth_registry_procedure` | `unknown` | Verify the current process on the official source. | Scope-limited to a routine birth in an Israeli hospital. The hospital notice is not itself the Population Registry entry; do not infer eligibility, parentage, documents, citizenship/residency, or any special-case process. |
...
- **Summary:** Usually system-led after a hospital birth in Israel. The hospital
  submits the birth notice and the Population and Immigration Authority
  completes the Population Registry entry.
...
- The direct primary procedure has a dated human-review decision: **Approved
  ...
  special paths or turn this candidate material into a runtime pack.
